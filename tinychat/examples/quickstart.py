...
class HistoryProcessor(MessageProcessor):
    ...
    async def _process(self, message: Message) -> Optional[Message]:
        ...
        if isinstance(message, (UserMessage, AIMessage)):
            ...
            print(
                f"📚 [History] Stored message #{len(self.messages)}: {message.content[:50]}..."
            )
        return message
...
class AIAssistantProcessor(MessageProcessor):
    ...
    async def _process(self, message: Message) -> Optional[Message]:
        ...
        if isinstance(message, UserMessage):
            ...
            intent = (
                message.metadata.get("intent", "unknown")
                if message.metadata
                else "unknown"
            )
            ...
            if intent == "greeting":
                ...
                response_text = (
                    f"I understand. You mentioned: '{message.content[:30]}...'"
                )
...
class LoggingObserver(Observer):
    ...
    async def on_message_received(self, data: MessageReceived):
        print(
            f"📥 [Observer] Processor '{data.processor.name}' received message: {data.message.id}"
        )

    async def on_message_processed(self, data: MessageProcessed):
        duration_ms = data.duration_ns / 1_000_000
        print(
            f"✅ [Observer] Processor '{data.processor.name}' processed in {duration_ms:.2f}ms"
        )

...
async def test_causal_lineage_basic():
    ...
    # Create a message
    msg = UserMessage(
        content="Hello", service=LLMServiceType.OPENAI, conversation_id="conv_1"
    )
...
async def test_processor_path_tracking():
    ...
    composite = CompositeProcessor(name="test_composite", processors=[proc])
    await composite.setup(setup)

    # Create a message
    msg = UserMessage(
        content="Test", service=LLMServiceType.OPENAI, conversation_id="conv_1"
    )
...
async def test_cycle_detection():
    ...
    composite = CyclicComposite(name="cyclic_composite", processors=[proc_a, proc_b])
...
async def test_max_hops_exceeded():
    ...
    composite = LongChainComposite(
        name="chain_composite", processors=processors, max_hops=max_hops
    )
...
async def test_no_cycle_with_different_paths():
    ...
    composite = CompositeProcessor(
        name="test_composite", processors=[proc_a, proc_b, proc_c]
    )
